...
- [x] **I-3: Catastro multi-ciudad** — `api/integrations/territorio.py` (dispatcher `verificar_territorio(lat, lon, ciudad)` con caché por celda) + endpoint `GET /api/v1/geo/catastro?ciudad=...`. **Verificado en vivo**: Barranquilla (catastro ArcGIS, NUPRE real), Medellín (POT servidormapas — capa ClasificacionSuelo), Cali (IDESC WFS barrios), Bogotá (IDECA federado, capa CensoArbol; catastro distrital CEL sin API pública — pendiente).
...
- [x] **UI con generación async** — El botón "Generar y Descargar Dictamen" usa la cola QStash (async_=true + polling de trabajos con indicador de estado) y degrada automáticamente a generación síncrona si la cola no está disponible. Verificado: PDF en ~10 s vía cola. [Hecho 2026-09-02]
